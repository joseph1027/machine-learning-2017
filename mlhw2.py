...
def test(root,train_data):
	...
	for number in [1,5,10,100]:# k
		...
		for data_index in range(0,len(test_data)):#0~35
			...
			for k in range(0,number):#number+1
				NN,predict_class = knn(root,query_data)
				result_table[predict_class] = result_table[predict_class] + 1
				if(data_index >= 0 and data_index <= 2):
					result[data_index].append(NN)

			max_vote = 0
			for hash_index in range(len(class_name)):
				if result_table[class_name[hash_index]] > max_vote:
					max_vote = result_table[class_name[hash_index]]
					final_predict_class = class_name[hash_index]

			print('Origin:',origin_class,' Predict:',final_predict_class)
			if origin_class == final_predict_class:
				correct = correct + 1
		print('A KNN is Done', data_index,' accuracy is ',float(correct/36.0))
		for cnt in range(3):
			for out_index in range(len(result[cnt])):
				print(result[cnt][out_index]+' ',end="")
			print('')
...
def knn(root,query_data):
	...
	while cur_node is not None:
		...
		cur_distance = distance(query_data,cur_node)
	...
	while record:
		...
		cur_distance = distance(query_data,parent_node)
		if parent_node.left_child is None and parent_node.right_child is None:
			if cur_distance < min_distance and parent_node.searched == 0:
				nearest = parent_node
				min_distance = cur_distance
		else:
